...
def parse_playlist_item():
    ...
    """
    file = io.open("data/neteasy_playlist_recommend_data.csv", 'a', encoding='utf8')
    for i in range(1, 5176):
        with io.open("data/{0}.json".format(i), 'r', encoding='UTF-8') as load_f:
...
def parse_playlist_id_to_name():
    file = io.open("data/neteasy_playlist_id_to_name_data.csv", 'a', encoding='utf8')
    for i in range(1, 5176):
        with io.open("data/{0}.json".format(i), 'r', encoding='UTF-8') as load_f:
            load_dict = json.load(load_f)
            try:
...
def parse_song_id_to_name():
    file = io.open("data/neteasy_song_id_to_name_data.csv", 'a', encoding='utf8')
    for i in range(1, 5176):
        with io.open("data/{0}.json".format(i), 'r', encoding='UTF-8') as load_f:
            load_dict = json.load(load_f)
            try:
